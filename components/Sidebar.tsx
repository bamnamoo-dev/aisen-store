...
const AI_NAV_ITEMS = [
  { name: 'AI 행정 챗봇', href: 'https://chatbot.aisen.store', icon: <Bot size={17} />, isExternal: true, badge: '3-Tier' },
  { name: '구글 챗봇 모음', href: '/chatbot', icon: <MessageSquare size={17} />, isExternal: false },
  { name: '스마트 여비정산기', href: 'https://chatbot.aisen.store/travel', icon: <Navigation size={17} />, isExternal: true, badge: 'v4.9.2' },
];
...
const WORK_NAV_ITEMS = [
  ...
  { name: '업무 소통 게시판', href: '/board', icon: <LayoutDashboard size={17} />, isExternal: false },
];
    ...
    <>
      ...
      <header className="md:hidden fixed top-0 left-0 right-0 h-14 bg-white border-b border-slate-200 z-50 flex items-center justify-between px-4">
        <Link href="/" className="flex items-center gap-2">
          ...
          <span className="text-base font-bold text-slate-900">아이센스토어</span>
          ...
          <div className="flex flex-col gap-1.5">
            ...
            {allNavItems.map((item) => (
              <Link 
                ...
                className="flex items-center justify-between p-3 rounded-xl text-slate-800 text-sm font-semibold hover:bg-blue-50"
              >
          ...
          <div className="pt-4 border-t border-slate-200 flex justify-between items-center text-sm">
            {user ? (
              ...
              <Link 
                href="/login" 
                onClick={() => setMobileMenuOpen(false)}
                className="w-full text-center py-2.5 rounded-xl bg-blue-600 text-white font-bold text-sm shadow-xs"
              >
                로그인
              </Link>
            )}
      ...
      <aside className="hidden md:flex fixed inset-y-0 left-0 w-64 bg-white border-r border-slate-200 z-40 flex-col justify-between p-5 shadow-xs">
        ...
        <div className="flex flex-col gap-6">
          ...
          <Link href="/" className="flex items-center gap-3 px-1 py-1 group">
            <div className="w-8 h-8 bg-blue-600 rounded-xl flex items-center justify-center text-white font-black text-base shadow-sm shadow-blue-500/20">
              i
            </div>
            <div className="flex flex-col">
              <div className="flex items-center gap-1.5">
                <span className="text-base font-extrabold text-slate-900 group-hover:text-blue-600 transition-colors tracking-tight">
                  아이센스토어
                </span>
              ...
              <span className="text-[11px] font-semibold text-slate-400">AI-SEN HUB v4.9.2</span>
            </div>
          </Link>

          {/* Group 1: AI 솔루션 */}
          <div className="flex flex-col gap-1">
            <p className="text-[11px] font-bold text-slate-400 px-3 uppercase tracking-wider mb-1">
              AI 핵심 솔루션
            </p>
            {AI_NAV_ITEMS.map((item) => {
              ...
              return (
                <Link 
                  ...
                  target={item.isExternal ? '_blank' : undefined}
                  rel={item.isExternal ? 'noopener noreferrer' : undefined}
                  className={`flex items-center justify-between px-3 py-2 rounded-xl text-[13px] font-semibold transition-all ${
                    isActive 
        ...
        <div className="flex flex-col gap-3 pt-3 border-t border-slate-200">
          ...
          <div className="p-3 rounded-xl bg-slate-50 border border-slate-200/80 text-[11px] text-slate-500 leading-relaxed">
            ...
            <span>카카오 길찾기 및 오피넷 실시간 유가 1일 6회 자동 연동</span>
          </div>
          ...
          {user ? (
            <div className="p-2.5 rounded-xl bg-white border border-slate-200 flex items-center justify-between shadow-xs">
              <div className="flex flex-col overflow-hidden">
                ...
                <span className="text-[10px] text-slate-400">교직원 인증 계정</span>
              </div>
              <button 
            ...
            <Link 
              href="/login" 
              className="w-full flex items-center justify-center gap-2 py-2 px-3 rounded-xl bg-blue-600 hover:bg-blue-700 text-white text-xs font-bold transition-all shadow-xs"
            >
              <LogIn size={14} />
