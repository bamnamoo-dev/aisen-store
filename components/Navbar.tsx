...
import { Bot, Navigation, FileText, Zap, LayoutDashboard, LogIn, LogOut, User, Menu, X } from 'lucide-react';
import { useState, useEffect } from 'react';
import { supabase } from '@/lib/supabase';

const NAV_ITEMS = [
  { name: 'AI 챗봇', href: 'https://chatbot.aisen.store', icon: <Bot size={15} />, isExternal: true },
  { name: '출장비 계산기', href: 'https://chatbot.aisen.store/travel', icon: <Navigation size={15} />, isExternal: true },
  { name: '행정 자료실', href: '/archive', icon: <FileText size={15} />, isExternal: false },
  { name: '미니 프로그램', href: '/tools', icon: <Zap size={15} />, isExternal: false },
  { name: '업무 게시판', href: '/board', icon: <LayoutDashboard size={15} />, isExternal: false },
];
...
export default function Navbar() {
  ...
  return (
    <nav className="glass-nav fixed top-0 left-0 right-0 z-50 h-14">
      <div className="max-w-[1140px] mx-auto px-4 md:px-6 h-full flex items-center justify-between">
      ...
      {mobileMenuOpen && (
        <div className="md:hidden bg-slate-950/95 border-b border-slate-800 px-5 py-4 flex flex-col gap-2">
          {NAV_ITEMS.map((item) => (
            <Link 
              key={item.href}
              href={item.href}
