...
- **스마트 원가계산서 검증기 (`/tools/cost-audit`)**: 조달청 2026 간접공사비 100% 역산 감사, 30일 미만 건강/연금 부당계상 0원 강제 차단, 3중 파서(엑셀/PDF/Gemini 3.5 Flash-Lite 비전) 지원, **2026 조달청 표준 수식형 서식(.xlsx)** 및 **감사결과 대차대조표 엑셀(.xlsx)** 다운로드 탑재
...
## 🚀 향후 핵심 추진 계획 (Roadmap): "데이터가 흐르는 행정 올인원 생태계"

AI-SEN 포털은 개별 도구의 단순 나열(Toolbox)에 머무르지 않고, **도구 간 데이터 공유(Data Flow)와 유기적 연계를 통해 학교 행정의 가치를 극대화**하는 방향으로 발전합니다:

1. **아키텍처 분업화의 확립**:
   * **AI 두뇌 (헤비 RAG)**: 102권 지침서 벡터 DB와 국가법령 연동은 **독립 고성능 VPS(chatbot.aisen.store)**가 전담.
   * **메인 허브 & 실무 툴킷**: 초고속 대문, 계약나침반, 여비, 식대, 원가계산 등 모든 웹 도구는 **Vercel(isen.store)**에 집결하여 무중단·초고속·무비용 확장성 유지.
2. **도구 간 데이터 공유 (Data Synergy) 파이프라인**:
   * **공사원가계산서 ➔ 계약나침반**: 원가계산서에서 산출된 최종 추정가격이 계약나침반으로 자동 전달되어 2인 견적/소액수의 판정 및 구비서류 자동 세팅.
   * **여비정산 + 계약견적 ➔ '학교 행정 임시 보관함'**: 출장여비와 계약 금액을 임시 보관함에 담아 **K-에듀파인 지출품의서 세트 원클릭 일괄 생성**.
   * **계약나침반 ➔ AI 행정 챗봇 (API 연계)**: 계약 판정 결과에 대한 세부 유권해석 질의를 AI 챗봇에 파라미터로 전달하여 공식 지침서 쪽수 1:1 검증.
3. **규정 및 요율 중앙화 (Centralized DB)**:
   * 연초(1~3월) 개정되는 조달청 제비율, 유류비, 소액수의 한도액을 Supabase DB에서 일원화 관리하여 포털 내 모든 도구가 동시 자동 갱신되는 유지보수 체계 확립.
