...
function generateRequiredDocsList(category, methodId, price, typeCode) {
  ...
  if (typeCode === 'B02') { // 교복구매 (정확한 매칭)
    ...
    docs.push({
      ...
      required: true,
      exemptible: false,
      note: '아동·청소년의 성보호에 관한 법률 제56조 (학생 대면 필수)'
    });
    docs.push({
      stage: stage1,
    ...
    docs.push({
      ...
      required: true,
      exemptible: false,
      note: 'TS한국교통안전공단 발행 (차령, 검사, 사고이력 조회)'
    });
    docs.push({
      stage: stage1,
    ...
    if (price >= 10000000) {
      docs.push({
        ...
        required: true,
        exemptible: false,
        note: '중소기업자간 경쟁제품 추정가격 1천만원 이상 시 징구'
      });
    }
    ...
    docs.push({
      ...
      required: true,
      exemptible: false,
      note: '1인당 1억원 이상 보상 한도 가입'
    });
    docs.push({
      stage: stage1,
      name: '숙박시설 안전 점검 서류 일체',
      required: true,
      exemptible: false,
    ...
    docs.push({
      stage: stage1,
      name: '안전요원 성범죄·아동학대 범죄전력조회 동의서 (법정 필수)',
      required: true,
      exemptible: false,
    ...
    docs.push({
      stage: stage1,
      name: '안전요원(안전지도사) 자격증 사본 및 재직증명서',
      required: true,
      exemptible: false,
      note: '소방청/대한적십자사 등 공인 안전교육 이수자'
    });
