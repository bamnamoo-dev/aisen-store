...
export default function HomePage() {
  ...
  return (
    <div className="w-full max-w-[1320px] mx-auto px-6 sm:px-10 py-5 sm:py-7 flex flex-col gap-6">
      ...
      <section className="flex flex-col items-center text-center pt-2 pb-1">
        ...
        {/* Big Wide Omnibar Search Box */}
        <div className="w-full max-w-[880px] mt-5">
          <form onSubmit={handleSearch} className="relative flex items-center">
            ...
            <input 
              ...
              className="w-full bg-white text-slate-900 border-2 border-slate-300 hover:border-slate-400 focus:border-blue-500 rounded-2xl pl-14 pr-36 py-3.5 sm:py-4.5 text-base sm:text-lg focus:outline-none focus:ring-4 focus:ring-blue-100 transition-all shadow-sm font-medium"
            />
            <button 
              type="submit" 
              className="absolute right-2.5 btn-primary py-2.5 sm:py-3 px-5 sm:px-6 text-sm sm:text-base font-bold shadow-xs rounded-xl"
            >
              <span>질의하기</span>
              <ArrowRight size={18} />
            </button>
          ...
          <div className="flex flex-wrap items-center justify-center gap-2.5 mt-3.5">
            ...
            {QUICK_SEARCH_CHIPS.map((chip, idx) => (
              <button
                ...
                className="inline-flex items-center gap-1.5 text-xs sm:text-sm px-3.5 py-1 rounded-xl bg-white text-slate-700 border border-slate-200 hover:border-blue-400 hover:text-blue-600 hover:bg-blue-50/50 font-semibold transition-all shadow-xs"
              >
                <span>{chip.icon}</span>
      ...
      {/* ========================================================
          2. CORE 4 SERVICES: 균일 높이 4분할 인터랙티브 카드
      ======================================================== */}
      <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <a 
          ...
          className="glass-card p-4.5 flex flex-col justify-between min-h-[160px] group hover:border-blue-400 hover:shadow-md transition-all cursor-pointer bg-white"
        >
          <div>
            ...
            <p className="text-xs sm:text-[13px] text-slate-500 mt-1 line-clamp-1">
              카카오 3개 경유지 및 오피넷 유가 연동 출장비 산출
            </p>
          </div>

          <div className="mt-3 pt-2.5 border-t border-slate-100 flex items-center justify-between text-xs sm:text-sm text-blue-600 font-bold">
            <span>정산기 바로가기</span>
        ...
        <a 
          ...
          target="_blank" 
          rel="noopener noreferrer"
          className="glass-card p-4.5 flex flex-col justify-between min-h-[160px] group hover:border-indigo-400 hover:shadow-md transition-all cursor-pointer bg-white"
        >
          <div>
            <div className="flex items-center justify-between mb-2.5">
            ...
            <p className="text-xs sm:text-[13px] text-slate-500 mt-1 line-clamp-1">
              28개 서고 지침서 1:1 쪽수 원본 뷰어 &amp; 국가법령 연동
            </p>
          </div>

          <div className="mt-3 pt-2.5 border-t border-slate-100 flex items-center justify-between text-xs sm:text-sm text-indigo-600 font-bold">
            <span>AI 질의하기</span>
            <ChevronRight size={15} className="group-hover:translate-x-1 transition-transform" />
          </div>
        </a>
        ...
        <Link 
          href="/archive" 
          className="glass-card p-4.5 flex flex-col justify-between min-h-[160px] group hover:border-emerald-400 hover:shadow-md transition-all cursor-pointer bg-white"
        >
          <div>
            <div className="flex items-center justify-between mb-2.5">
            ...
            <p className="text-xs sm:text-[13px] text-slate-500 mt-1 line-clamp-1">
              28개 분야 102권 공식 지침서 스트리밍 및 실무 서식
            </p>
          </div>

          <div className="mt-3 pt-2.5 border-t border-slate-100 flex items-center justify-between text-xs sm:text-sm text-emerald-600 font-bold">
            <span>자료실 바로가기</span>
            <ChevronRight size={15} className="group-hover:translate-x-1 transition-transform" />
          </div>
        </Link>
        ...
        <Link 
          href="/board" 
          className="glass-card p-4.5 flex flex-col justify-between min-h-[160px] group hover:border-sky-400 hover:shadow-md transition-all cursor-pointer bg-white"
        >
          <div>
            <div className="flex items-center justify-between mb-2.5">
            ...
            <p className="text-xs sm:text-[13px] text-slate-500 mt-1 line-clamp-1">
              가입 없이 닉네임과 4자리 비밀번호로 실무 질의 및 팁 공유
            </p>
          </div>

          <div className="mt-3 pt-2.5 border-t border-slate-100 flex items-center justify-between text-xs sm:text-sm text-sky-600 font-bold">
            <span>게시판 바로가기</span>
            <ChevronRight size={15} className="group-hover:translate-x-1 transition-transform" />
          </div>
        </Link>
      ...
      <section className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        ...
        <div className="glass-panel p-5 bg-white border border-slate-200 shadow-xs flex flex-col justify-between">
          <div>
              ...
              {recentPosts.length > 0 ? (
                recentPosts.map((post) => (
                  <Link 
                    key={post.id} 
                    href="/board" 
                    className="flex items-center justify-between p-3 rounded-xl bg-white hover:bg-blue-50/50 transition-all border border-slate-200/90 hover:border-blue-300 shadow-2xs hover:shadow-xs group"
                  >
                ...
                <div className="py-6 text-center text-xs sm:text-sm text-slate-400">등록된 게시물이 없습니다.</div>
              )}
            </div>
          </div>
        </div>
              ...
              {recentDocs.length > 0 ? (
                recentDocs.map((doc) => (
                  <Link 
                    key={doc.id} 
                    href="/archive" 
                    className="flex items-center justify-between p-3 rounded-xl bg-white hover:bg-emerald-50/50 transition-all border border-slate-200/90 hover:border-emerald-300 shadow-2xs hover:shadow-xs group"
                  >
                    <div className="flex items-center gap-2.5 overflow-hidden">
                ...
                <div className="py-6 text-center text-xs sm:text-sm text-slate-400">등록된 서식이 없습니다.</div>
              )}
            </div>
          </div>
        </div>
