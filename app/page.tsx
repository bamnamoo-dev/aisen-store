...
export default function HomePage() {
  ...
  return (
    <div className="w-full max-w-[1260px] mx-auto px-6 py-6 flex flex-col gap-6">
      ...
      <section className="flex flex-col items-center text-center pt-2 pb-2">
        ...
        <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white border border-slate-200 text-xs text-slate-600 mb-3 shadow-xs">
          ...
          <span className="font-bold text-slate-800">AI-SEN v4.9.2 엔진 가동중</span>
          <span className="text-slate-300">|</span>
          <Fuel size={13} className="text-amber-500" />
          <span className="text-slate-600">오피넷 실시간 유가 1일 6회 연동</span>
        </div>

        {/* Headline */}
        <h1 className="text-2xl sm:text-3.5xl font-black text-slate-900 tracking-tight leading-snug">
        ...
        <p className="text-xs sm:text-sm text-slate-500 mt-1 font-medium max-w-[600px]">
          102권 공식 지침서 1:1 앵커링 3-Tier RAG 챗봇과 카카오·오피넷 실시간 연동 스마트 여비정산기
        </p>

        {/* Wide Omnibar Search Box */}
        <div className="w-full max-w-[820px] mt-5">
          <form onSubmit={handleSearch} className="relative">
            ...
            <input 
              ...
              placeholder="궁금한 교육행정 지침(수의계약, 강사료, 회계 등)이나 출장비, 서식을 검색하세요..."
              className="w-full bg-white text-slate-900 border border-slate-300 hover:border-slate-400 focus:border-blue-500 rounded-2xl pl-12 pr-28 py-3.5 text-sm sm:text-base focus:outline-none focus:ring-4 focus:ring-blue-100 transition-all shadow-sm"
            />
            <button 
              type="submit" 
              className="absolute right-2 top-1/2 -translate-y-1/2 btn-primary py-2 px-4.5 text-xs sm:text-sm font-bold shadow-xs"
            >
              <span>질의하기</span>
              <ArrowRight size={15} />
            </button>
          ...
          <div className="flex flex-wrap items-center justify-center gap-2 mt-3">
            <span className="text-xs text-slate-400 font-semibold">추천 질문:</span>
            {QUICK_SEARCH_CHIPS.map((chip, idx) => (
              <button
                key={idx}
                onClick={() => handleChipClick(chip.query)}
                className="text-xs px-2.5 py-0.5 rounded-lg bg-white text-slate-600 border border-slate-200 hover:border-blue-400 hover:text-blue-600 transition-colors shadow-xs"
              >
                {chip.label}
              </button>
            ))}
      ...
      <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        >
          <div>
            ...
            <p className="text-xs text-slate-500 mt-1.5 leading-relaxed line-clamp-2">
              카카오 3개 경유지 길찾기와 오피넷 유가 연동 출장비 산출 및 A4 공문서 인쇄
            </p>
          </div>
        ...
        <a 
          ...
          target="_blank" 
          rel="noopener noreferrer"
          className="glass-card p-5 flex flex-col justify-between group hover:border-indigo-400 hover:shadow-md transition-all cursor-pointer bg-white"
        >
          <div>
            <div className="flex items-center justify-between mb-3">
            ...
            <p className="text-xs text-slate-500 mt-1.5 leading-relaxed line-clamp-2">
              28개 서고 지침서 100% 원본 쪽수 뷰어(#page=N) 및 국가법령센터 조문 연동
            </p>
          </div>
        ...
        <Link 
          href="/archive" 
          className="glass-card p-5 flex flex-col justify-between group hover:border-emerald-400 hover:shadow-md transition-all cursor-pointer bg-white"
        >
          <div>
            <div className="flex items-center justify-between mb-3">
        ...
        <Link 
          href="/board" 
          className="glass-card p-5 flex flex-col justify-between group hover:border-sky-400 hover:shadow-md transition-all cursor-pointer bg-white"
        >
          <div>
            <div className="flex items-center justify-between mb-3">
          ...
          <div className="mt-4 pt-3 border-t border-slate-100 flex items-center justify-between text-xs text-sky-600 font-bold">
            <span>게시판 바로가기</span>
            <ChevronRight size={14} className="group-hover:translate-x-1 transition-transform" />
          </div>
        </Link>
      ...
      <section className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        ...
        <div className="glass-panel p-6 bg-white border border-slate-200 shadow-xs">
          <div className="flex items-center justify-between mb-3 pb-3 border-b border-slate-100">
            ...
            <div className="flex items-center gap-2">
              <Link href="/board" className="text-xs text-slate-500 hover:text-blue-600 font-bold">
                더보기 &rarr;
              </Link>
            </div>
          </div>
            ...
            {recentPosts.length > 0 ? (
              recentPosts.map((post) => (
                <Link 
                  key={post.id} 
                  href="/board" 
                  className="flex items-center justify-between p-3 rounded-xl bg-slate-50 hover:bg-slate-100/80 transition-colors border border-slate-100 text-xs sm:text-sm group"
                >
        ...
        <div className="glass-panel p-6 bg-white border border-slate-200 shadow-xs">
          <div className="flex items-center justify-between mb-3 pb-3 border-b border-slate-100">
            ...
            <div className="flex items-center gap-2">
              <Link href="/archive" className="text-xs text-slate-500 hover:text-emerald-600 font-bold">
                더보기 &rarr;
              </Link>
            </div>
          </div>
      ...
      {/* Footer */}
      <footer className="w-full pt-4 pb-2 border-t border-slate-200 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs text-slate-400">
        <div>
