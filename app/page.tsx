...
export default function HomePage() {
  ...
  return (
    <div className="w-full max-w-[1240px] mx-auto px-6 py-6 flex flex-col gap-5">
      
      {/* ========================================================
          1. TOP COMMAND & SEARCH HERO (Standard Clean Banner)
      ======================================================== */}
      <section className="glass-panel p-6 sm:p-7 bg-white border border-slate-200 flex flex-col gap-4">
        ...
        <form onSubmit={handleSearch} className="relative w-full mt-1">
          ...
          <input 
            ...
            placeholder="궁금한 교육행정 지침(수의계약, 강사료, 회계규칙 등)이나 출장비, 서식을 검색하세요..."
            className="w-full bg-slate-50 hover:bg-white focus:bg-white text-slate-900 border border-slate-300 rounded-xl pl-11 pr-28 py-3 text-sm focus:outline-none focus:border-blue-500 focus:ring-3 focus:ring-blue-100 transition-all shadow-inner"
          />
          <button 
            type="submit" 
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 pt-3 border-t border-slate-100">
          {METRICS.map((metric, idx) => (
            <div key={idx} className="flex flex-col p-1.5 rounded-lg bg-slate-50/70 border border-slate-100 text-center">
      ...
      {/* ========================================================
          2. CORE 4 SERVICES (2x2 Uniform Balanced Grid)
      ======================================================== */}
            ...
            <div className="flex items-center justify-between gap-2 mb-2">
              <div className="flex items-center gap-2.5">
                ...
                <h3 className="text-base font-bold text-slate-900">
                  스마트 여비정산기 &amp; 출장 유류비 계산기
                </h3>
              </div>
              <span className="text-[11px] font-bold text-blue-600 bg-blue-50 px-2 py-0.5 rounded border border-blue-100 shrink-0">
                v4.9.2
              </span>
            </div>
        ...
        <div className="glass-card p-5 flex flex-col justify-between">
          ...
          <div className="flex items-center gap-2 pt-3 border-t border-slate-100">
            <a 
              ...
              target="_blank" 
              rel="noopener noreferrer" 
              className="btn-primary bg-indigo-600 hover:bg-indigo-700 flex-1 py-2 text-xs font-bold"
            >
        ...
        <div className="glass-card p-5 flex flex-col justify-between">
          <div>
            <div className="flex items-center justify-between gap-2 mb-2">
              ...
              <span className="text-[11px] font-bold text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded border border-emerald-100 shrink-0">
                102권 공식 지침
              </span>
            </div>
          ...
          <div className="flex items-center gap-2 pt-3 border-t border-slate-100">
            <Link 
              href="/archive" 
              className="btn-secondary w-full py-2 text-xs justify-center font-bold"
            >
              <span>자료실 바로가기</span>
              <ChevronRight size={14} />
            </Link>
        ...
        <div className="glass-card p-5 flex flex-col justify-between">
          <div>
            <div className="flex items-center justify-between gap-2 mb-2">
              ...
              <span className="text-[10px] font-bold text-sky-600 bg-sky-50 px-2 py-0.5 rounded border border-sky-100 shrink-0">
                미가입 작성
              </span>
            </div>
      ...
      {/* ========================================================
          3. REAL-TIME FEEDS (2-Column Dashboard Cards)
      ======================================================== */}
      <section className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div className="glass-panel p-5 bg-white border border-slate-200">
          <div className="flex items-center justify-between mb-3 pb-2.5 border-b border-slate-100">
            ...
            <Link href="/board" className="text-xs text-slate-500 hover:text-blue-600 font-semibold">
              더보기 &rarr;
            </Link>
          </div>
              ...
              recentPosts.map((post) => (
                ...
                >
                  <div className="flex items-center gap-2.5 overflow-hidden">
                    <span className="text-[10px] px-2 py-0.5 rounded bg-blue-50 text-blue-700 font-bold shrink-0">
                      {post.category || '공지'}
                    </span>
        ...
        <div className="glass-panel p-5 bg-white border border-slate-200">
          <div className="flex items-center justify-between mb-3 pb-2.5 border-b border-slate-100">
            ...
            <Link href="/archive" className="text-xs text-slate-500 hover:text-emerald-600 font-semibold">
              더보기 &rarr;
            </Link>
          </div>
          ...
          <div className="flex flex-col gap-2">
            {recentDocs.length > 0 ? (
              recentDocs.map((doc) => (
                ...
                >
                  <div className="flex items-center gap-2.5 overflow-hidden">
                    <span className="text-[10px] px-2 py-0.5 rounded bg-emerald-50 text-emerald-700 font-bold shrink-0">
                      {doc.category || '서식'}
                    </span>
                    <span className="text-slate-800 font-medium truncate">{doc.file_name}</span>
                  </div>
                  <span className="text-[11px] text-slate-400 shrink-0 ml-2">
