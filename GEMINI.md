...
1. **원스크린(One-Screen) 컴팩트 뷰포트 (4 × 3 그리드)**: 
   - 메인 화면(`app/page.tsx`)은 데스크톱 1080p 해상도에서 상단 검색 옴니바와 12개 카드가 스크롤 없이 한눈에 들어오도록 최적화된 여백과 폰트 크기(`h-[150px]`)를 유지합니다.
...
3. **클라이언트 0초 무부하 아키텍처**:
   - 모든 엑셀 변환, 인쇄, 도면 작업은 서버 업로드 없이 사용자 웹브라우저 메모리 안에서 0초 만에 처리하여 보안성과 무료 티어 용량 무제한 유지를 보장합니다.
